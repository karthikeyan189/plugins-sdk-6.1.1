...
<form method="POST" action='<portlet:actionURL><portlet:param name="pagesize" value="${model.pageSize}"/><portlet:param name="pagenumber" value="${model.currentPageNumber}"/></portlet:actionURL>'>
	...
	<div class="datarow">
		<div class="box">
			...
			<spring:bind path="model.facilityId">
				<select name="facilityId" value="<c:out value="${model.user.objectCoordinator.facilityId}" />" width="290px" style="width:290px" >
					<OPTION value="<c:out value="0" />"><c:out value="-- Facility --" /></OPTION>
					<c:forEach items="${model.facilitiesList}" var="facility">
							<OPTION value="${facility.id}" <c:if test="${facility.id==model.user.objectCoordinator.facilityId}"> <c:out value="selected" /> </c:if> > <c:out value="${facility.name}" /></OPTION>
					</c:forEach>
				</select>
	...
	<div class="row">
	    ...
	    <input type="hidden" name="action" value="approvalcoordinator">
 	</div>
</form>