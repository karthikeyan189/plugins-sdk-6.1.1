...
<form:form action="${actionURL}" commandName="model">
    ...
    <form:hidden path="id" />
		...
		<div class="datarow">
			<div class="dynamicRow" style="margin-top:10px;">
				<div id="facultySelect" style="float:left;">
					...
					<select size="1" >
						...
						<c:forEach items="${model.allFaculties}" var="faculty">
							<option value="${faculty.key}" <c:if test="${fn:startsWith(faculty.value, '----') == false}">disabled="disabled"</c:if> >${faculty.value}</option>
						</c:forEach>
					</select>
				</div>
			</div>
...
function <portlet:namespace />extractCodeFromEditor() {
	...
	jQuery(document).ready(function(){
		...
		jQuery('.addEventTypeButton').click(function(){
			jQuery('#eventTypeSelect select option:selected').each(function(){
				if(jQuery(this).text()!=""){
					var length = jQuery('#eventTypesContainer input').length
					var str = '<div class="dynamicRow">';
					str += '<div class="formtitle" >'+jQuery(this).text()+'</div><button type="button" class="deleteRowButton" onclick="var p = jQuery(this).parent();p.remove()">x</button>'
					str += '<input type="hidden" name="eventTypes[\''+jQuery(this).val()+'\']" id="eventTypes\''+jQuery(this).val()+'\'" value="'+jQuery(this).text()+'" />';
					str += '</div>';
					
					jQuery('#eventTypesContainer').html(jQuery('#eventTypesContainer').html() + str);
					jQuery(this).removeAttr('selected');
				}
			});
		});
